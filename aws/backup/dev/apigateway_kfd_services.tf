# Private API Gateway for KFD Services
...
# Public API Gateway for KFD Services
resource "aws_api_gateway_rest_api" "kfd_api_regional" {
  name        = "${var.api_kfd_name}_regional_${var.env}"
  description = "Regional API Gateway For KFD Services"
  endpoint_configuration {
    types = ["REGIONAL"]
  }
  lifecycle {
...
resource "aws_api_gateway_resource" "kfd_services_regional" {
  rest_api_id = aws_api_gateway_rest_api.kfd_api_regional.id
  parent_id   = aws_api_gateway_rest_api.kfd_api_regional.root_resource_id
  path_part   = "services"
}

resource "aws_api_gateway_resource" "kfd_services_id_regional" {
  rest_api_id = aws_api_gateway_rest_api.kfd_api_regional.id
  parent_id   = aws_api_gateway_resource.kfd_services_regional.id
  path_part   = "{id}"
}

resource "aws_cloudwatch_log_group" "kfd_api_log_group_regional" {
  name              = "/aws/apigateway/${var.api_kfd_name}_regional_${var.env}"
  retention_in_days = var.logs_retention_days
  kms_key_id        = data.aws_kms_alias.cw.target_key_arn
}

resource "aws_cloudwatch_log_group" "kfd_api_execution_log_group_regional" {
  name              = "API-Gateway-Execution-Logs_${aws_api_gateway_rest_api.kfd_api_regional.id}/${var.env}"
  retention_in_days = var.logs_retention_days
  kms_key_id        = data.aws_kms_alias.cw.target_key_arn
}

resource "aws_api_gateway_method" "kfd_put_method_root_regional" {
  rest_api_id   = aws_api_gateway_rest_api.kfd_api_regional.id
  resource_id   = aws_api_gateway_resource.kfd_services_regional.id
  http_method   = "PUT"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "kfd_put_method_root_integration_regional" {
  rest_api_id             = aws_api_gateway_rest_api.kfd_api_regional.id
  resource_id             = aws_api_gateway_resource.kfd_services_regional.id
  http_method             = aws_api_gateway_method.kfd_put_method_root_regional.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${module.kfd_api_lambda.lambda_function_arn}/invocations"
}

resource "aws_api_gateway_method" "kfd_delete_method_services_id_regional" {
  rest_api_id   = aws_api_gateway_rest_api.kfd_api_regional.id
  resource_id   = aws_api_gateway_resource.kfd_services_id_regional.id
  http_method   = "DELETE"
  authorization = "NONE"
  request_parameters = {
    "method.request.path.id" = true
...
resource "aws_api_gateway_integration" "kfd_delete_method_services_id_integration_regional" {
  rest_api_id             = aws_api_gateway_rest_api.kfd_api_regional.id
  resource_id             = aws_api_gateway_resource.kfd_services_id_regional.id
  http_method             = aws_api_gateway_method.kfd_delete_method_services_id_regional.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${module.kfd_api_lambda.lambda_function_arn}/invocations"
}

resource "aws_api_gateway_deployment" "kfd_deployment_regional" {
  depends_on = [
    aws_api_gateway_method.kfd_put_method_root_regional,
    aws_api_gateway_method.kfd_delete_method_services_id_regional,
    aws_api_gateway_integration.kfd_put_method_root_integration_regional,
    aws_api_gateway_integration.kfd_delete_method_services_id_integration_regional
  ]
  rest_api_id = aws_api_gateway_rest_api.kfd_api_regional.id
  lifecycle {
    create_before_destroy = true
...
resource "aws_api_gateway_stage" "kfd_stage_regional" {
  rest_api_id          = aws_api_gateway_rest_api.kfd_api_regional.id
  deployment_id        = aws_api_gateway_deployment.kfd_deployment_regional.id
  stage_name           = var.env
  description          = "Api Stage for ${var.api_kfd_name}_regional_${var.env}"
  xray_tracing_enabled = true
  lifecycle {
    ignore_changes = [deployment_id]
  }
  ...
  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.kfd_api_log_group_regional.arn
    format          = "$context.extendedRequestId $context.identity.sourceIp $context.identity.caller $context.identity.user [$context.requestTime] \"$context.httpMethod $context.resourcePath $context.protocol\" $context.status $context.responseLength $context.requestId"
  }
}

resource "aws_api_gateway_method_settings" "kfd_all_regional" {
  rest_api_id = aws_api_gateway_rest_api.kfd_api_regional.id
  stage_name  = aws_api_gateway_stage.kfd_stage_regional.stage_name
  method_path = "*/*"
  settings {
    metrics_enabled      = true
    logging_level        = "INFO"
...
resource "aws_lambda_permission" "kfd_upload_trigger_regional" {
  statement_id  = "AllowApiGatewayPutRegional"
  action        = "lambda:InvokeFunction"
  function_name = module.kfd_api_lambda.lambda_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.kfd_api_regional.id}/*/PUT/services"
}

resource "aws_lambda_permission" "kfd_delete_trigger_regional" {
  statement_id  = "AllowApiGatewayDeleteRegional"
  action        = "lambda:InvokeFunction"
  function_name = module.kfd_api_lambda.lambda_function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "arn:aws:execute-api:${var.region}:${data.aws_caller_identity.current.account_id}:${aws_api_gateway_rest_api.kfd_api_regional.id}/*/DELETE/services/*"
}

output "kfd_api_gateway_id_regional" {
  value = aws_api_gateway_rest_api.kfd_api_regional.id
}

output "kfd_api_gateway_root_resource_id_regional" {
  value = aws_api_gateway_rest_api.kfd_api_regional.root_resource_id
}
