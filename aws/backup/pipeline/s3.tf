module "core_pipeline_bucket" {
  source                      = "../modules/s3"
  bucket_name                 = local.s3_bucket_pipeline
  tags                        = local.tags
  enable_versioning           = true
  enable_logging              = true
  logging_target_bucket       = module.s3_core_audit_logs.bucket_id
  logging_target_prefix       = "core_pipeline_bucket/"
  environment                 = var.env
...
resource "aws_s3_bucket_ownership_controls" "core_pipeline_bucket_ownership" {
  bucket = module.core_pipeline_bucket.bucket_id
  rule {
