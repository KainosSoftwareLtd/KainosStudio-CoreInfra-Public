...
data "aws_subnet" "private_subnets_staging" {
  vpc_id = data.aws_vpc.staging_vpc.id
  id     = var.staging_private_subnet_id

}

data "aws_security_group" "staging-codebuild_sg" {
  id = var.staging_codebuild_security_group_id
}
