# KFD Services API (matching AWS API Gateway KFD Services structure exactly)
resource "azurerm_api_management_api" "kfd_services" {
  ...
  path                  = "" # Root path to match AWS exactly
  protocols             = ["https"]
...
# KFD Services API - Upload Operation (AWS: PUT /)
...
# KFD Services API - Delete Operation (AWS: DELETE /services/{id})
...
# Apply upload policy to the upload operation
...
# Apply delete policy to the delete operation
